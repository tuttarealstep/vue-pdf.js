...
import { OnPasswordCallback, PDFSource, PDFSourceOptions, PDFSourceWithOptions, UpdatePasswordFn } from "../types"
...
export async function parseSourceFile(source: PDFSource): Promise<PDFDocumentLoadingTask> {
    ...
    if (typeof source === 'string') {
        ...
        if (source.startsWith('data:application/pdf;base64,')) {
            return getDocument(new Uint8Array(atob(source.split(',')[1])
                .split('')
                .map(function (c) {
...
export async function processSource(inputSource: PDFSource | PDFSourceWithOptions, sourceOptions?: PDFSourceOptions): Promise<PDFDocumentProxy | null> {
    if (!inputSource) {
        return null;
    }
    ...
    const loadingTask = await parseSourceFile(source);
